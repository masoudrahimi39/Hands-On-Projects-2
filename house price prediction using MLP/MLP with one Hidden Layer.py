...
#___________________________________________ not normalizing label

# data_frame = pd.read_csv('House Sales.csv')
# train_label = data_frame.values[:4000, 2]
# test_label = data_frame.values[4000:5000, 2]
# data = data_frame.values[:, 3:]
# #######   normalizing data   ###########
# for i in range(18):
#     data[:, i] = (data[:, i]-data[:, i].min())/(data[:, i].max()-data[:, i].min())
